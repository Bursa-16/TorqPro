...
- **Faz 2.6.2B (2026-07-23):** `FrictionConditionRecord` now carries 18 records (overall/combined coefficient only, `friction_model = combined_or_unspecified`) -- deterministically re-homed from already-approved `CoatingRecord`/`LubricationRecord` ISO 16047/ISO 4042 typical-range data, not new measurements. None of these can feed Section 5's `mu_G`/`mu_K` split yet (no independent thread/bearing value exists for any of them); Section 5 remains fed by direct API input only, unchanged from Faz 2.6.0.
- **Faz 2.6.3 (2026-07-23):** investigated whether any formula in this codebase can consume a single combined friction coefficient (`overall_friction_coefficient_min/max`) as-is. Finding: **no** -- Section 4's `M_A = K*d*F_M` needs `K` (none of the 18 records has one, and `K` may never be derived from `mu` per SS4 of this document / `docs/12_CLAUDE_CONTEXT.md`); Section 5's `mu_G`/`mu_K` decomposition needs independent thread/bearing values (none exist), and assigning the combined value to both would be an unjustified physical assumption, explicitly forbidden this phase. Neither formula changed. A new, additive `backend.calculation_engine.friction_readiness` module was added instead: it resolves a `friction_condition_id`, reports calculation-mode readiness (`mode_a_combined_estimate` / `mode_b_separated_model` / `blocked`), and computes a *friction-coefficient* min/nominal/max sensitivity range (nominal = arithmetic midpoint of the reference range, explicitly labelled as such) -- never a torque value, and never a thread/bearing split. See `docs/phases/PHASE_2.6.3_FRICTION_AWARE_TORQUE_MODEL.md`.
